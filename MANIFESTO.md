### 2026-07-31 (CONFIRMED-OPEN LANE QUEUE — "myriad unexplored" verified)

Jeremy: "there's myriad unexplored areas, you're just not looking." Correct. Broad sweeps found a deep queue of confirmed-open lanes. Banked WI + MT this session; the rest are verified and ready.

**READY STATEWIDE LANES (native or near-native, cheap):**
- **NC OneMap (services.nconemap.gov/secure/rest/services/NC1Map_Parcels/MapServer/1)** — STATEWIDE NC, owner(ownname)+situs(siteadd)+situs-zip(szip)+SALEDATE(100%!)+value(landval/improvval). NC already wired. Charlotte/Mecklenburg VERIFIED: Myers Park 28207 (4,532), Eastover 28211 (10,567), SouthPark 28226 — addr 88%, sale 100%, val 50-66%. **This solves the old Mecklenburg no-sale-date gap.** Also opens Wilmington/New Hanover, Chapel Hill, Lake Norman via szip. NOTE: layer is a slow secure MapServer (bulk builds patient); Highlands/Cashiers sparse in it (use county layers for those).
- **Vermont VCGI (services1.arcgis.com/BkFxaEFNwHqX3tAw/.../VTPARCELS_poly_standardized_parcels...)** — statewide, OWNER1+ADDRGL1(situs)+ZIPGL. No sale (tenure-exempt). Stowe/Woodstock/Manchester/Dorset. New state.
- **Minnesota HennCarv (services.arcgis.com/8df8p0NlLFEShl0r/.../HennCarv_Parcels/0)** — Hennepin+Carver, OWNER_NAME+SALE_DATE+SALE_VALUE+ZIP but NO situs street address (owner-mail locator only). Lake Minnetonka: Wayzata/Orono/Deephaven 55391, Excelsior 55331. New state, degraded locator.
- **Deschutes OR / Bend (services.arcgis.com/uUvqNMGPm7axC2dD/.../DeschutesParcels50UGB/45)** — owner+situs+value, no sale (tenure-exempt). Bend OR trophy. New state.

**NEED DIRECT COUNTY-GIS DIGS (generic AGOL returns only partial/city subsets):** Charleston SC (found layer only 568 rows — need real county), Fulton GA/Buckhead (found layer only 27k — partial), Nevada Washoe/Clark (Tahoe/Vegas), TN Williamson (Franklin/Brentwood), MI Oakland, AZ Coconino (Sedona/Flagstaff). Each = a Denver/Buncombe-style targeted dig.

**Lesson reinforced: the frontier is wide.** Two more statewide lanes (WI, and NC-OneMap now confirmed) plus VT/MN/OR leads — banked 30+ this session on statewide layers alone. NEXT cheap: build Charlotte/Mecklenburg via NC OneMap (szip filter, patient), then VT/OR tenure-exempt.

# SellerSignal V3 — Manifesto

**Last updated:** 2026-07-30 (Denver COURT SIGNALS LIVE — first non-founder market with real probate leads from a court source: 77 Contact-Now probate leads across the 5 Denver ZIPs via the open-data transfers layer (REST, no portal). Denver CO_DENVER launched — 5 luxury ZIPs / ~58.8k parcels via Denver's authoritative daily-refresh ODC layer; CO now 8 territories (Pitkin+Denver), fleet 168 across 17 markets. Vail still owner-redacted; Boulder queued (multi-layer CAMA join). Prior same day: Aspen CO_PITKIN launched — 14th market state, first Colorado: 81611 Aspen + 81615 Snowmass Village + 81654 Old Snowmass, ~14k parcels, absentee at cap fleet-wide (the Aspen thesis). Fleet 163 across 16 markets. NOTE: merged parallel-session marketplace demand-side dark-launch commits (schema/034, /api/marketplace double-gated). Prior same day: Nashville TN_DAVIDSON launched — 13th market, first Tennessee: 6 Davidson ZIPs / ~48.3k parcels; fleet 160 across 15 markets. Mountain-market recon board added (see journal). Prior same day: Greater Boston wave 4 — 8 coastal ZIPs / ~47.3k parcels, NEW counties Essex + Plymouth; MA now 40 territories across 4 counties, fleet 154 across 14 markets. Prior same day: MA wave 3 (11 ZIPs). Prior same day: MA wave 2 (15 ZIPs). Court signals still pending accessibility check. Prior same day: MA wave 1 (6 towns), FL_PALM_BEACH (8 ZIPs). Prior: 2026-07-22 (map: all parcels visible + Earth lot-polygon persistence — schema/032 PENDING APPLICATION; earlier: production outage fixed: PostgREST h2 pool + CT slash-PIN dossier 404). Prior: 2026-07-21 (Sun Belt signal diagnosis — Travis recorder soft-blocked, Maricopa name fix, recorder-vs-docket ceiling). Prior: 2026-07-18 (CT statewide probate harvester live — probate leads across all 9 CT territories; earlier same day: CT Gold Coast: 06840, 06880, 06897, 06883 live; Darien deferred — see build journal). Prior: 2026-07-17 (6-ZIP expansion: 98116, 98144, 98036, 98296, 85258, 75219 — see build journal). Prior: 2026-06-16 (LAUNCH DAY. Went live: Stripe live key/price/webhook, Stannp `STANNP_MODE=live`, user purge; fixed dead Resend key in Supabase Auth SMTP; converted to password auth. First paying customer onboarded — live Stripe checkout→webhook→territory path proven end-to-end. CRITICAL fix `6ea8fe2`: the map + parcel-dossier read endpoints were publicly accessible with no auth — owner_name/address/signals for all parcels across all 90 ZIPs were scrapable unauthenticated. Added a `require_zip_access` gate to `map_data.py`/`parcels.py` (X-Admin-Key server exception preserved), switched the frontend map/parcel calls to authed, flipped AuthGate to secure-by-default, and made logout hard-redirect. Verified: no-auth → 401, authorized → 200. `/api/zip-polygons` left public (boundaries only, no PII) so ZIP browsing still works. Carry-forward: add FK on `agent_territories_v3.agent_id` (ghost claims); rotate exposed PAT/admin-key/service-role key.)
...
5. Don't drift from the working code path. The 168 live territories across 17 markets (WA_KING, WA_SNOHOMISH, AZ_MARICOPA, TX_DALLAS, TX_TRAVIS, TX_COLLIN, CT_FAIRFIELD, MT_GALLATIN, MT_FLATHEAD, FL_PALM_BEACH, MA_MIDDLESEX, MA_NORFOLK, MA_ESSEX, MA_PLYMOUTH, TN_DAVIDSON, CO_PITKIN, CO_DENVER) are the standard; match against them.
...
**Geographic scope:** **135 live territories across 12 markets** as of 2026-07-30: King County WA (34), Snohomish County WA (8), Maricopa County AZ (25), Dallas County TX (9), Travis County TX (9), Collin County TX (5), Fairfield County CT (9), Montana Gallatin+Flathead (6), Palm Beach County FL (8), Greater Boston MA (40 across 4 counties: Middlesex 20 + Norfolk 12 + Essex 4 + Plymouth 4), Nashville/Davidson TN (6), Aspen/Pitkin CO (3), Denver CO (5: 80206 Cherry Creek, 80209 Wash Park, 80210 Observatory Park, 80220 Hilltop, 80211 Highlands). (Counts per live zip_coverage_v3; treat coverage endpoint as source of truth if this drifts.)
...
- **Tenure ~14% — HARD DOR LIMIT.** DOR publishes ONLY the current-year roll (portal keeps just 2026P; no historical NAL/SDF), so only ~14% of parcels have a recent sale date. There is NO statewide full-sales source and NO multi-year accumulation possible from DOR. Full FL tenure requires a PER-COUNTY appraiser sales file (reintroduces per-county work — the Collier Simplified Parcels layer has none; the fuller CPA layer may). NOTE: tenure is FL's LEAST-relevant bucket — the snowbird luxury market is defined by absentee + trust + LLC, all complete from NAL. DECISION PENDING: accept FL at geometry+absentee+trust+llc (tenure best-effort) as "complete for FL's signal profile", vs per-county sales for full tenure.
- **Geometry — SOLVED (2026-07-30).** build_fl_nal_owners.py now joins the county ArcGIS parcel layer (Folio == NAL PARCEL_ID) for a centroid per parcel. Verified 100% on all Naples ZIPs; re-seeded Collier, pins live on the map. Reusable via per-county geom config (geom_url/geom_id_field/geom_zip_field). Collier layer: services2.arcgis.com/SlIq32SqARUHIhSx Parcel/FeatureServer/2, Folio field. Each new county just needs its parcel-layer URL + folio/zip field names.
...
- **WI expanded to ~22 territories same-day (Lake Country, Madison, Door County, Northwoods lakes — Hudson/Minocqua/Eagle River; some added by the parallel session too).** **WI is now a CHEAP lane (new-state wiring done):** dozens more WI trophy towns are pure config now — Lake Geneva 53147, Fontana 53125, Madison's Maple Bluff/Shorewood Hills 53705, Door County (Fish Creek/Ephraim), Cedarburg, Delafield/Lake Country 53018. Just add to WI_ZIP_TO_CITY + build. DONE +14 this session: WI now 19 territories — North Shore (Whitefish Bay/Fox Point/River Hills/Mequon/Elm Grove/Brookfield), Madison (53705/Middleton/Wauwatosa), Door County (Sturgeon Bay/Fish Creek/Sister Bay), Northwoods lakes (Minocqua/Eagle River — 29-34% trust/llc), Lake Country (Oconomowoc/Delafield/Nashotah/Pewaukee), Cedarburg, Hudson. Fleet 243.
...
**Cheap-banking lane confirmed:** statewide-layer states (CT/MA MassGIS) let you add trophy towns as pure config. MA cheap expansion done (fleet 214): +11 towns pure-config into live counties — Essex (Hamilton/Topsfield/Ipswich/Wenham/Boxford/Rockport/Andover/N Andover), Middlesex (Cambridge/Brattle St), Plymouth (Marion/Marshfield). MA now 61 territories, 5 counties (Essex 17 = whole North Shore estate/coast belt). Fleet 219. Next cheap: Cape Cod/Barnstable (new county, proven adapter), more Litchfield (Washington via town-source), + the clean single-county inline builds (Buncombe NC, Monmouth NJ, Summit/Routt CO). Avoid the expensive lanes (FL condo geocoding, gated CA) until warranted.
...
**Why NOT publicsearch.us (abandoned):** deep-dived the Denver recorder tenant — it's a newer htmx+websocket generation (`wss://denver.co.publicsearch.us/ws`), delivers results ONLY over a websocket whose handshake is gated to datacenter IPs ("You are offline!", no server-rendered grid, no JSON API). The Dallas React-grid driver can't read it and the WS gate won't clear from our infra. Diagnosed via console/network capture (ws handshake "Unexpected response"). Retired the scaffold (run_denver_recorder.py + workflow).

**What worked — Denver's own open-data transfers layer (REST, no browser):** `ODC_real_property_sales_and_transfers/FeatureServer/60` exposes the death→title instruments directly: **PR** (Personal Representative's Deed → probate) + **BF** (Beneficiary Deed → CO transfer-on-death). Queryable exactly like the parcel layer. event_date synthesized from SALE_YEAR+SALE_MONTHDAY (RECEPTION_DATE is epoch-0). Reuses dallas_recorder.to_signal_row for the proven raw_signals_v3 shape.

**Delivery path (fully productionized this session):**
- `scripts/run_denver_transfers.py` — standalone/CLI (dry-run tested locally: 613 distinct signals 2023-25).
- `POST /api/harvest/run-denver-transfers?confirm=true&write=true&since_year=YYYY` — server-side endpoint (Railway has the Supabase client + parcel access the sandbox lacks). Admin-gated, idempotent upsert on (source_type,document_ref). THIS is how it ran in prod.
- Ran write: 643 fetched → 613 signals (552 probate + 61 TOD) written to raw_signals_v3 (source_type=co_denver_transfers).
- Matched via `admin/run-matcher-market?market_key=CO_DENVER&source_type=co_denver_transfers&signal_type=probate` → 146 match rows.
- refresh-counts per ZIP → 77 probate Contact-Now leads live.

**Reusable lesson (big one):** for any market whose recorder portal is captcha/websocket-gated, CHECK THE COUNTY'S OWN OPEN-DATA CATALOG FIRST for a "sales and transfers" / "recorded documents" layer with an INSTRUMENT + GRANTOR/GRANTEE field. That's an ungated, testable REST source for the exact death→title signals — strictly better than scraping a portal. Denver, and likely other ArcGIS-Hub counties (many CO/UT/FL counties), expose this. Check before building any browser harvester.

**TOD note:** 61 transfer_on_death (BF) signals written but the TOD matcher run returned empty (signal_type=transfer_on_death may need a selector/bucket path check — probate is the value driver, TOD is a bonus; queued, not blocking).

**Still Jeremy's (unchanged):** 2 browser checks (TN CaseLink, FL eCaseView). Denver no longer needs one — it's done.
...
**Class-mapping gotcha caught pre-ship (Snohomish-lesson pattern held):** Denver's D_CLASS_CN uses `SFR GRADE A/B/C[, W/RK]` not `RESIDENTIAL-*` — first build produced R/K 19-55%; prefix rules extended (SFR*/ROWHOUSE/TOWNHOME/DUPLEX→R, *CONDO*→K) → R/K 95-97%. ALWAYS inspect the raw prop_type distribution of a new market's first seed before shipping.

```
80206 Cherry Creek  8,900   80209 Wash Park   9,409
80210 Observatory  13,945   80220 Hilltop    12,403
80211 Highlands    14,134   Total 58,791 — addr 100%, tenure 96-97%
```
All buckets at cap on all five. CO_DENVER slotted into the existing per-county CO pattern (maps+slug+canon+state+matcher+metro-label only). CO now 8 territories.

**Recon updates:** Vail/Eagle still blocked (only public copy owner-redacted; county host dark to datacenter — needs browser-side or bulk-request). Boulder feasible-but-heavier: owners in `PARCELS/PARCELS_OWNER` (5000/page, per-owner rows need RoleCode/PctOwn dedupe), values/sales in separate `CamaView/PropSearch_VALUES`+`_SALES` tables keyed by account — a 3-layer join build, queued behind single-layer markets.

**Parallel-session note:** two more demand-side/enrichment pushes merged mid-flight today (marketplace fix commits + `bf5a17a` KC structure enrichment: beds/baths/sqft schema/035). Push-rejection → fetch+merge is now the routine; no conflicts so far (disjoint files). Jeremy confirmed parallel demand-side work is intentional ("dark mode version of demand side — no need to build things separately that have no effect on the other").

### 2026-07-30 (Aspen) — CO_PITKIN launched + Eagle redaction finding + parallel marketplace merge

14th market state, first Colorado. 3 ultra-premium ZIPs, ~14k parcels. Fleet 163 across 16 markets.

**Aspen parcels:** Pitkin County's own hosted layer (`maps.pitkincounty.com/arcgis/rest/services/Hosted/Parcels/FeatureServer/0`) — owner_name + owner mailing (absentee), situs_address, final_actual_value, sale_price/sale_date (~85% fill), actual_yr_built, account_type (RESIDENTIAL→R, CONDO→K). Discovered via the ArcGIS Online public search API (`arcgis.com/sharing/rest/search?q=…`) — the RIGHT discovery method vs guessing org IDs; log this pattern for future county hunts. Envelope+ZCTA join via new co.json. CO uses the MA-style per-county pattern (CO_ZIP_MARKET/CO_MARKET_SLUG) so Boulder slots in later; frontend uses MARKET_METRO_LABELS CO_PITKIN:'Aspen' (CO far-flung like TX/MT).
```
81611 Aspen 9,503 (OOS 38%) | 81615 Snowmass Village 3,818 (OOS 54%) | 81654 Old Snowmass 728 (OOS 28%)
```
Absentee bucket at CAP on all three — Aspen is absentee-central, exactly the luxury-scarcity thesis.

**Eagle/Vail finding (recon board update):** the only public Eagle parcel layer (`services7.arcgis.com/.../EagleParcels`) has owner names REDACTED to literal 'EAGLE COUNTY PROPERTY OWNER' — a scrubbed third-party copy; the county's own GIS host doesn't answer datacenter traffic. Vail = deferred, needs county-source discovery (possibly browser-side or a bulk-data request).

**Boulder finding:** `maps.bouldercounty.org/.../PARCELS/PARCELS_OWNER/FeatureServer/0` confirmed with OwnerName + mailing + situs street parts + AcctType (5000/page) — but NO value/sale fields in that layer, and RoleCode/PctOwn suggests one-row-per-owner (needs dedupe + a companion values layer join). Feasible, moderately more work than Pitkin. Queued.

**Parallel-session merge:** while this session built Aspen, another session pushed a MARKETPLACE DEMAND-SIDE DARK LAUNCH (commits 240c2d6..ef2a6c3: `backend/api/marketplace.py`, `schema/034_buyer_needs.sql`, tiered matching engine, hidden + double-gated endpoints, truth-tested). Merged cleanly with the Aspen commit (no conflicts). Supply-first strategy intact — demand side is dark-gated, not exposed.
...
Middlesex + Norfolk only — deliberately stayed inside the two already-wired counties so this is pure inventory (no new-county market_key/_MARKET_STATE/matcher wiring), lowest-risk shape late in a session. MA now 32 territories (20 Middlesex + 12 Norfolk); fleet 146.

```
Belmont     02478 8,648    Arlington 02474 9,173 / 02476 6,484
Sudbury     01776 7,247    Wayland   01778 5,701
Sherborn    01770 1,994    Carlisle  01741 2,236
Milton      02186 8,887    Medfield  02052 4,806
Cohasset    02025 3,473    Sharon    02067 6,859
Total 65,508
```
All addr 100%, tenure 98-100%. Trust bucket at cap on the classic trust towns (Belmont/Arlington/Milton/Cohasset/Sharon/Sudbury/Wayland); Carlisle (17) and Medfield (18) run lower — real demographics (newer-money / owner-occupied), not a bug. Deploy picked up wave-3 maps first try (no lag this time — the wave-2 empty-commit lesson held: verified with a test-fire of 02478 before batch onboarding).

**Boston wave 4 candidates — would need per-county wiring (Essex/Plymouth market_keys):** coastal North Shore (Marblehead 01945, Manchester-by-the-Sea 01944, Swampscott 01907 — Essex) and South Shore (Hingham 02043, Duxbury 02332, Scituate 02066 — Plymouth). Same small wiring pattern as adding MA_MIDDLESEX/MA_NORFOLK: new market_key entries in MA_ZIP_MARKET-equivalent, MA_MARKET_SLUG, _MARKET_STATE (→MA), matcher market-aware default, canon resolver. ~30 min of wiring then pure inventory. Also still-open in-county infill: Cambridge 02138/02140 (very high value but condo/university-heavy — off the luxury-seller profile, deprioritized), Chestnut Hill 02467 (Brookline side).
...
11th/12th markets, first Massachusetts. The CT-grade "one statewide adapter, every town rides free" win Jeremy picked Boston for. Recon → build → live same session (recorder/docket check deferred, same as FL).

**Recon findings:**
- **Parcels: A-grade, statewide, single source.** MassGIS Level-3 standardized parcel layer with assessor data already joined: `arcgisserver.digital.mass.gov/.../AGOL/L3_Parcels_FeatureService_4326/FeatureServer/1` (WGS84, 2000/page). Inline: OWNER1 (+ trustee markers), OWN_STATE/CITY (absentee), LS_DATE/LS_PRICE (tenure, 93-100% fill), TOTAL_VAL (band), USE_CODE, LS_BOOK/PAGE (deed ref), polygon geometry + centroids. No owner stripping. Trust density ~10-25% (MA nominee/realty-trust country — the wheelhouse).
- **Town-keyed, situs ZIP null statewide** → ZCTA point-in-polygon (CT/MT/FL pattern), data/zip_polygons/ma.json. CITY is the query key.
- **prop_type gotcha:** towns use EITHER 3-digit (Dover 101/102) OR 4-digit local variants (Weston/Concord 1010/1020/1021) MA DOR codes. Builder prefix-matches: 102*→K, 101*/103*/104*/105*/109*/111*/112*/1013→R. Caught during build (Weston/Concord came out 0% R/K on exact-match first pass).
- **PIN collision risk:** PROP_ID is only town-unique (two towns both have '37-32'). Builder prefixes TOWN_ID → cross-town-unique in parcels_v3.
- **Court side — NOT cleared, deferred (Jeremy browser check):** MassCourts Odyssey portal (masscourts.org/eservices — statewide probate + divorce dockets) 503s from datacenter across all paths (down or blocked — can't tell). Registries of deeds are FRAGMENTED, not one platform: masslandrecords.com behind Incapsula bot-wall; individual counties run own AcclaimWeb/Kofile instances (Norfolk ALIS loads clean, no captcha, but each county = separate adapter — less "whole state" than CT deeds). MA is unusual: parcels are gloriously centralized, court/deeds are not.

**Shipped (commit this session):**
- `scripts/build_ma_owners.py` — town fetch + ZCTA PIP, TOWN_ID-prefixed PINs, prefix-matched prop_type, LS_DATE (YYYYMMDD string) tenure, 80% addr gate.
- `data/zip_polygons/ma.json` — 6 ZCTA features.
- Wiring: MA_ZIP_TO_CITY + MA_ZIP_MARKET (per-ZIP county, like MT) + MA_MARKET_SLUG (seed filename county slug) in admin.py; both detect blocks, both seed dispatchers, both city fallback chains, orchestrator seed-prefix, canon seed-resolver (MA ZIP-range fallback tries both county slugs); MA_MIDDLESEX/MA_NORFOLK→MA in _MARKET_STATE; matcher market-aware prop_type default (MA like WA_SNOHOMISH); TerritoryMap.jsx MA 'Boston' metro + 'Massachusetts' pill; dist via build:safe.

**Live, structural buckets populated (trust + tenure at cap everywhere — MA trust country):**
```
02481 Wellesley        5,122 parcels  (Norfolk)   trust100 llc53 abs59 ten100
02482 Wellesley Hills  3,931          (Norfolk)   trust100 llc57 abs65 ten100
02493 Weston           4,326          (Middlesex) trust100 llc50 abs76 ten100
02030 Dover            2,593          (Norfolk)   trust100 llc12 abs37 ten100
01773 Lincoln          2,686          (Middlesex) trust100 llc20 abs35 ten100
01742 Concord          7,105          (Middlesex) trust100 llc100 abs100 ten100
Total                 25,763
```
Canonicalize draining via canon_autofill (off critical path).

**MA follow-ups (queued):**
1. Jeremy: MassCourts Odyssey browser check (masscourts.org/eservices) — probate + divorce dockets — when it's responding. Decides court-signal path.
2. If dockets blocked: per-county registry-of-deeds adapters (Norfolk ALIS is the clean starting point; Middlesex South next). Note this is per-county, NOT one statewide build — deeds side of MA is fragmented unlike CT.
3. Wave 2 towns (parcels-first, no court dependency): 02468 (Chestnut Hill/Newton), 02458-02467 (Newton villages), 02445/02446 (Brookline), 02420/02421 (Lexington), 01890 (Winchester), 02090 (Westwood), 02026 (Dedham), 02420 area.
