...
**OPEN: Flathead 2025 tail.** One `courts=flathead year=2025 max_cases=400` dispatch after TSPD cools (≥12h spacing suggested) walks from ~298; expect the run to end on miss_streak=25 (genuine frontier, likely ~350-400) rather than an error ABORT. Then rematch drains automatically; run refresh-counts on the 6 MT ZIPs. Daily cron covers 2026 only and will not pick this up.
