...
**Last updated:** 2026-06-15 (institutional-owner leakage fixed: tenure buckets were surfacing convents/HOAs/utilities/veterans-posts at the TOP because `_derive_owner_type` mis-tagged them `individual`. Expanded the classifier (denominations/fraternal/utilities/associations, surname-collision-guarded) + added an 80yr individual-tenure plausibility cap as a name-independent backstop; reclassified + refreshed all 90 ZIPs; verified clean (Greenwich + KING luxury ZIPs: 0 non-individuals). Follow-up: sync markers into the 5 script builders. Prior 2026-06-14 evening: pre-launch readiness audit PASS; TX condo map geometry 92→99%/82→98%; recorder runners hardened with per-chunk flush.)
