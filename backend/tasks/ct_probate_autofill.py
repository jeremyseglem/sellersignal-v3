"""
CT Probate daily new-case report autofill task.
...
daily cadence, since CT Probate County Clerk publishes new reports once
per business day (after court close).

Behavior:
  - Ticks every TICK_INTERVAL seconds (default 24h = 86400s).
  - Each tick harvests the last CTPROBATE_SINCE_DAYS days of reports. The
  ...
  - First tick fires CTPROBATE_STARTUP_DELAY seconds after boot (default 90s).
  - On error, uses exponential backoff up to 60 min.
  - State exposed via /api/harvest/ct-probate-autofill-status.
  - Can be paused/resumed via /api/harvest/ct-probate-autofill-
    {pause,resume}.

Why a single daily tick (vs obit's 12h cadence):
  CT Probate publishes reports once per business day. Polling more often
...
KC Superior Court portal. CT Probate's source is a static PDF endpoint —
...
TICK_INTERVAL      = int(os.environ.get("CTPROBATE_AUTOFILL_TICK_SECONDS", "86400"))   # 24h
CTPROBATE_SINCE_DAYS     = int(os.environ.get("CTPROBATE_AUTOFILL_SINCE_DAYS",   "10"))
CTPROBATE_RUN_TIMEOUT    = int(os.environ.get("CTPROBATE_AUTOFILL_RUN_TIMEOUT",  "600"))    # 10min
CTPROBATE_STARTUP_DELAY  = int(os.environ.get("CTPROBATE_AUTOFILL_STARTUP_DELAY", "90"))
...
_ZIP_FILTER_RAW    = os.environ.get("CTPROBATE_AUTOFILL_ZIP_FILTER", "").strip()
...
state: dict = {
    ...
    "config": {
        ...
        "since_days":       CTPROBATE_SINCE_DAYS,
...
async def _run_one_tick(client: httpx.AsyncClient, admin_key: str) -> dict:
    """Fire one CT Probate daily harvest cycle and return the parsed result."""
    r = await client.post(
        "/api/harvest/run",
        json={
            "source":          "ct_probate",
            "since_days_ago":  CTPROBATE_SINCE_DAYS,
            "dry_run":         False,
            "match_after":     True,
            "zip_filter":      ZIP_FILTER,
        },
        headers={"X-Admin-Key": admin_key},
        timeout=CTPROBATE_RUN_TIMEOUT,
    )
    r.raise_for_status()
    return r.json()


async def ct_probate_autofill_loop() -> None:
    """Main task body. Runs until cancelled."""
    admin_key = os.environ.get("ADMIN_KEY", "").strip()
    if not admin_key:
        log.error(
            "ct_probate_autofill: ADMIN_KEY env var not set — disabling. "
    ...
    log.info(
        f"ct_probate_autofill: scheduled to tick every {TICK_INTERVAL}s "
        f"(since_days={CTPROBATE_SINCE_DAYS}, zip={ZIP_FILTER}), "
        f"first tick in {CTPROBATE_STARTUP_DELAY}s"
    )
    await asyncio.sleep(CTPROBATE_STARTUP_DELAY)
    ...
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        while True:
            try:
                ...
                log.info(
                    f"ct_probate_autofill tick #{state['total_ticks']}: "
                ...
                log.info("ct_probate_autofill: loop cancelled (shutdown)")
                raise
                ...
                log.error(
                    f"ct_probate_autofill tick failed "
