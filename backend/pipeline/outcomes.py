...
# ═══════════════════════════════════════════════════════════════════════
# SANDBOX-BOUND PATH — OPTION Y ANNOTATION
# ═══════════════════════════════════════════════════════════════════════
# OUTCOMES_PATH below is hardcoded to the v2 sandbox filesystem. It is
# consumed by load_outcomes() and save_outcomes() — which therefore
# also inherit the sandbox coupling.
#
# THE THREE PURE FUNCTIONS ARE THE PORTABLE SURFACE AREA:
#   - register_surfaced(outcomes, picks, week_of) → updated outcomes dict
#   - get_excluded_pins(outcomes) → set of excluded pins
#   - status_for(outcomes, pin) → status string
# These take the outcomes dict as an argument and operate in memory.
# V3 callers should load/save the outcomes dict via their own mechanism
# (Supabase table, parameterized path, etc.) and pass it to these pure
# functions.
#
# The two I/O functions (load_outcomes, save_outcomes) are preserved
# verbatim for v2 reference. They will NOT work in v3 as-is because
# /home/claude/sellersignal_v2/out/ does not exist outside the sandbox.
# ═══════════════════════════════════════════════════════════════════════
