"""
SellerSignal v3 — King County render_playbook orchestrator (v2 reference port).

Renders the weekly plays as a formatted PDF from this-weeks-picks.json.
Uses reportlab for layout and the Lora font family.

STATUS: NOT RUNNABLE IN V3 AS-IS.
Hardcoded paths in the __main__ block point at /home/claude/sellersignal_v2/out/.
Module-top font registration calls also reference absolute paths at
/usr/share/fonts/truetype/google-fonts/Lora-*.ttf — so IMPORTING this
module can fail if those font files are not present.

DEPENDENCIES:
  - reportlab (external Python package)
  - pypdf (external Python package, used for page count)
  - Lora-Variable.ttf and Lora-Italic-Variable.ttf at the paths on L19-20

When v3 ships to a different deployment environment, this module may
need font-path parameterization. Deferred until that deployment exists.

CONTAINS:
  - KC-specific: ZIP_TO_CITY dict (6 Eastside ZIPs), absolute font paths,
    __main__ block with v2 sandbox paths.
  - Parameterized: render(picks_path, out_path) is callable with any
    path, so v3's KC orchestrator can invoke it directly with Supabase-
    produced picks written to a tmpfile.

PURPOSE:
  1. Specification — documents the operator-playbook PDF format used
     by v2 and referenced in the 2026-04-18 14:36 UTC briefing.
