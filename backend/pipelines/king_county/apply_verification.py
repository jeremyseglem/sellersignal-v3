"""
SellerSignal v3 — King County apply_verification orchestrator (v2 reference port).

Post-banding obit-verification promotion/demotion layer. Reads the
banded-inventory.json produced by apply_banding and the obit-verification
results, then applies:

  - confirmed_* verification → Band 1A upgrade
  - no_match + obit-only signal → reject (drop from inventory)
  - no_match + other strong signals → demote to Band 2
  - unverified → leave as Band 2.5
  - cluster dissolution if all members rejected

STATUS: NOT RUNNABLE IN V3 AS-IS.
Hardcoded paths under /home/claude/sellersignal_v2/out/ only exist in
the v2 sandbox. Importing this module is safe — the procedural body is
wrapped in run_98004_verification_promotion() and guarded by __main__.

PURPOSE:
  1. Specification — documents exactly how v2 promoted/demoted obit-
     matched candidates post-banding.
  2. Migration target — when v3's KC orchestrator is written against
     Supabase, this file is the diff target.
...
def run_98004_verification_promotion():
    """
    Execute the v2 KC verification-promotion flow.
    Requires banded-inventory.json + obit-verification-results.json from the v2 sandbox.
    """
...
if __name__ == "__main__":
    run_98004_verification_promotion()
