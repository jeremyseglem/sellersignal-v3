"""
SellerSignal v3 — King County weekly_selector orchestrator (v2 reference port).

Deterministic weekly playbook generation: picks the 10 weekly leads from
banded-inventory-verified.json and writes this-weeks-picks.json.

STATUS: NOT RUNNABLE IN V3 AS-IS.
Hardcoded paths under /home/claude/sellersignal_v2/out/ only exist in
the v2 sandbox. Importing this module is safe — all procedural code
is inside function bodies or the __main__ guard.

CONTAINS A LOT OF LIBRARY-CALIBER LOGIC:
  Pure selection functions (library-caliber, currently living here):
    select_call_now, select_build_now, select_strategic_holds
    get_recent_pins, owner_base_key, resolve_copy
    _score, _has_blocker, _investigation_demotes_from_call_now,
    _investigation_promotes_to_call_now
  Copy templates (KC-neutral except PROPERTY_OVERRIDES):
    COPY_TEMPLATES, STRATEGIC_HOLDS_TEMPLATES
  KC-specific data:
    PROPERTY_OVERRIDES (5 entries, hand-crafted copy for specific
      Bellevue/Medina addresses)
    INV_PATH, HIST_PATH, PICKS_PATH (v2 sandbox paths)
  Sandbox orchestration:
    load_history, save_history, generate_weekly_playbook
    __main__ block with KC-specific ZIP→city label dict

These may eventually be split — pure selection functions to
backend/pipeline/, PROPERTY_OVERRIDES + orchestration here. Deferred
per user direction (port A now, evaluate split after 98004 pipeline
is running and proven, avoid up-front architecture decisions).

PURPOSE:
  1. Specification — documents exactly how v2 selected the weekly slate
     that produced the 2026-04-18 14:36 UTC 'pressure briefing'.
  2. Migration target — when v3's KC orchestrator reads from Supabase
     and persists picks+history+outcomes there instead of local JSON,
     this file is the diff target.
...
def generate_weekly_playbook(week_of=None):
    from backend.pipeline.outcomes import load_outcomes, register_surfaced, save_outcomes, get_excluded_pins
